 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class Happy_Sniffs_Strings_ConcatenationSpacingSniff implements PHP_CodeSniffer_Sniff
{
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if ($this->spacing === 0) {
            $message = 'Concat operator must not be surrounded by spaces ' . $this->spacing;
            $data    = array();
        } else {
            if ($this->spacing > 1) {
